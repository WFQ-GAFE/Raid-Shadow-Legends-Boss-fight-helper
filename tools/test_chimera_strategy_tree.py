...
def main() -> int:
    ...
    assert matches(hero_alive_condition, positioned)
    hero_dead_condition = copy.deepcopy(hero_alive_condition)
    hero_dead_condition["conditionTree"]["state"] = "dead"
    assert not matches(hero_dead_condition, positioned)
    dead_positioned = copy.deepcopy(positioned)
    dead_positioned["heroes"][1]["dead"] = True
    assert matches(hero_dead_condition, dead_positioned)
    assert not matches(hero_alive_condition, dead_positioned)
    validate_strategy_config(
        {
            "mode": "execute",
            "rules": [
                {
                    "name": "specific-hero-dead",
                    "when": hero_dead_condition,
                    "action": {
                        "type": "cast",
                        "skillSlot": 1,
                        "target": {"type": "boss"},
