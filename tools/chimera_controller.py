...
def match_condition_tree(
    ...
    *,
    ...
    if node_type == "group":
        ...
        hero_type_id = node.get("heroTypeId")
        team_position = node.get("teamPosition")
        candidates = [
            hero
            for hero in state_entities(state, "heroes")
            if hero.get("typeId") == hero_type_id
        ]
        if isinstance(team_position, int) and not isinstance(team_position, bool):
            positioned = [
                hero
                for hero in candidates
                if hero.get("teamPosition") == team_position
            ]
            # Older/partial live snapshots may not expose preparation slots.
            # A unique type identity is still safe; duplicate copies require
            # the exact annotated slot to avoid guessing the wrong champion.
            if positioned:
                candidates = positioned
            elif len(candidates) != 1:
                candidates = []
        desired_state = node.get("state")
        matched = bool(candidates) and any(
            (hero.get("dead") is True) == (desired_state == "dead")
            for hero in candidates
        )
    else:
        return False
