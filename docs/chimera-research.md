...
调查基线：RAID 11.70.0，`Raid.exe` 文件版本 6000.3.18.6209205，RSL Helper 9.27.2.1824。
...
- RSL Helper 主程序中有 394 个包含 `Chimera` 的可见字符串，说明它覆盖了队伍配置、试炼分析、评分和重开等外围功能。
- RSL Helper 的注入 DLL 中只有少量奇美拉专属入口；实际战斗状态和技能下发复用通用战斗代码。
...
RSL Helper 的注入模块还包含 `CAST fire: CreateCmdManually(target, skill)`，证明这条动作路径在当前版本中可用。不过我们不复用或分发它的闭源 DLL。

## 真实运行时探测结果

只读代理先在 Raid PID 75396 中完成安全重载测试，随后按窗口层级确认测试账户并迁移到 PID 73664。两个 PID 是两个独立账户，不能再用 RSL Helper 启动时间推断所属关系。实际类型位置如下：
